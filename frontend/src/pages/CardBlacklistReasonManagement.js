...
import '../styles/CardBlacklistReasonManagement.css';
import Layout from '../components/layout/Layout';
import {
    Search, Plus, Edit, Trash2, ChevronLeft, ChevronRight,
    AlertTriangle, Filter, X
} from 'lucide-react';
import EditCardBlacklistReasonModal from '../components/EditCardBlacklistReasonModal';
import ExportToExcel from '../components/ExportToExcel';

const CardBlacklistReasonManagement = () => {
    const { user } = useAuth();
    ...
    const fetchBlacklistReasons = async () => {
        try {
            ...
            console.log("Kart Kara Liste Sebepleri:", reasonsData);
            ...
            console.error('Kart kara liste sebepleri yüklenirken hata oluştu:', err);
            setError('Kart kara liste sebepleri yüklenirken bir hata oluştu. Lütfen tekrar deneyin.');
            setLoading(false);
        }
    };
    ...
    const handleConfirmDelete = async () => {
        try {
            await api.delete(`/api/card-blacklist-reasons/${reasonToAction.id}`);
    ...
    if (loading && blacklistReasons.length === 0) {
        return (
            <div className="blacklist-reason-loading">
                ...
                <p>Kart kara liste sebepleri yükleniyor...</p>
            </div>
        );
    }
    ...
    return (
        <Layout pageTitle="Kart Kara Liste Sebep Yönetimi">
            {/* Ana içerik */}
            <div className="blacklist-reason-tools">
                <div className="search-filter">
                    ...
                    <ExportToExcel
                        ...
                        fileName="Kart_Kara_Liste_Sebepleri"
            ...
            {/* Kart kara liste sebepleri tablosu */}
                        ...
                        {blacklistReasons.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan="3" className="no-data">
                                    {loading ? 'Veriler yükleniyor...' : 'Kart kara liste sebebi bulunamadı'}
                                </td>
                            </tr>
                        )}
                        </tbody>
            ...
            {/* Kart kara liste sebebi düzenleme modalı */}
            <EditCardBlacklistReasonModal
...
export default CardBlacklistReasonManagement;